...
const response = await axios.post(
  "https://api.flutterwave.com/v3/payments",
  {
    ...
    payment_options: method === "mobilemoney" ? "mobilemoneyrwanda,mobilemoneyuganda,mobilemoneyzambia" : "card",
    customer: {
  ...
  },
  {
    ...
    return new Response(
      JSON.stringify({ error: "Méthode inconnue" }),
      { status: 400 }
    );
  } catch (err) {
    console.error(err);
    ...
    return new Response(
      JSON.stringify({ error: "Erreur lors du paiement" }),
      { status: 500 }
    );
  }
}
