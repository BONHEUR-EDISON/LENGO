...
export default function PremiumBlogCarousel() {
  ...
  const cardWidth = 320; // largeur fixe des cartes

  // Scroller à gauche/droite
  ...
  // Mettre à jour l'index actif pour les dots
  ...
  return (
    <section
      className={`relative py-20 px-4 sm:px-8 md:px-16 transition-colors duration-500 ${
    ...
    >
      ...
      <button
        onClick={() => scroll('left')}
        className={`hidden md:flex absolute left-0 top-1/2 transform -translate-y-1/2 p-3 rounded-full shadow-lg transition-colors duration-300 ${
      ...
      <button
        ...
        className={`hidden md:flex absolute right-0 top-1/2 transform -translate-y-1/2 p-3 rounded-full shadow-lg transition-colors duration-300 ${
          isDark ? 'bg-gray-800 hover:bg-gray-700 text-white' : 'bg-gray-200 hover:bg-gray-300 text-black'
        }`}
        aria-label="Suivant"
      >
      ...
      <div
        ref={carouselRef}
        className="flex overflow-x-auto space-x-6 scrollbar-hide snap-x snap-mandatory scroll-smooth relative z-10"
      >
          ...
          >
            <motion.div
              className={`rounded-xl overflow-hidden shadow-lg transition-transform duration-300 hover:scale-105 ${
                isDark ? 'bg-gray-800 hover:shadow-2xl' : 'bg-gray-100 hover:shadow-2xl'
              }`}
            ...
            >
              ...
              <div className="p-6 flex flex-col justify-between h-full">
                ...
                <span
                  className={`mt-4 inline-block text-center py-2 px-4 rounded font-medium transition-colors duration-300 ${
