...
export default function LeftBar({ posts }: LeftBarProps) {
  ...
  return (
    <aside className="hidden lg:block w-80 sticky top-20 h-[calc(100vh-5rem)] overflow-y-auto p-4 space-y-4 border-l border-gray-200 dark:border-gray-700">
      ...
      <ul ref={containerRef} className="space-y-4">
        {posts.map((post, index) => (
          <li
            key={post.slug}
            data-index={index}
            className={`group relative cursor-pointer bg-white dark:bg-gray-800 rounded-md shadow-sm overflow-hidden transition-all duration-500
              ${visibleItems.has(index) ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}
            `}
          >
            <Link href={`/blog/${post.slug}`} className="flex flex-col md:flex-row gap-3 p-3">
              <div className="w-full md:w-24 h-24 relative flex-shrink-0 rounded-md overflow-hidden">
                <Image
                  src={post.image}
                  alt={post.title}
                  fill
